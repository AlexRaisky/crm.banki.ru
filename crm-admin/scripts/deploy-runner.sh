...
once() {
    ...
    # Пауза из панели. Отмечаться продолжаем — иначе панель решит, что обработчик умер,
    # и спрячет кнопки вместо того, чтобы честно показать «на паузе».
    if [ "$(psql_q "SELECT paused FROM app.deploy_runner WHERE id = 1" 2>/dev/null)" = "t" ]; then
        return 0
    fi
