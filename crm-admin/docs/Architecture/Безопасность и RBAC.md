...
`Sections.ALL` — 36 канонических id; список обязан совпадать с id пунктов NAV (`js/shell.js`, массив `NAV` — см. [[Оболочка панели (shell)]]) и панелей `/settings` (`PANES`, поле `acl`). Порядок в `ALL` — это порядок строк в матрице прав, поэтому он совпадает с порядком меню.
