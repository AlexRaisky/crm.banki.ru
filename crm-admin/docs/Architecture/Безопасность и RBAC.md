...
|---|---|---|---|
| **Супер админ** | супер | 10 | всё; роль скрыта из UI, привязана к списку почт в `ADMIN_EMAIL` |
...
## Деактивация роли вместо удаления (V40)

Удалить роль можно было только пока она не встроенная И на ней нет ни одной учётки (`RoleService.delete`) — то есть практически никогда: нужда «эта роль больше не используется» возникает как раз у ролей с людьми.

Теперь у роли есть `is_active`. Отключённая роль остаётся в справочнике вместе со своей матрицей (история «кто что мог» не теряется), но:

- новым учёткам не назначается — `UserService.resolveRole` отвечает 409 с именем роли;
- её носители **не входят в панель**: `AppUserPrincipal.isEnabled()` требует и включённой учётки, и активной роли. Гасить права по одному разделу было бы полумерой — человек вошёл бы и увидел пустое меню. Уже открытые сессии при этом не рвутся: проверка стоит на входе.

Запереть себя снаружи нельзя: супер-роль не отключается вовсе, а последнюю активную админ-роль сервер отдать откажется — иначе управлять доступом станет некому. Встроенность отключению не мешает: «Маркетолог» может стать ненужным ровно так же, как заведённая руками роль.

Ручка — `PUT /api/admin/roles/{id}/active` с телом `{"active": false}`. `DELETE /api/admin/roles/{id}` остался для полностью ошибочных записей, но панель им больше не пользуется.
...
2. `ADMIN_EMAIL` разбирается как **список** адресов (запятая, точка с запятой или пробел) — супер-админов может быть несколько;
3. носитель супер-роли, которого нет в списке, **понижается** до обычной админ-роли;
4. каждая учётка из списка либо повышается до супер-роли, либо создаётся заново (displayName «Администратор», `enabled = true`, пароль из `ADMIN_PASSWORD`, BCrypt; в лог — предупреждение сменить его). Пароль существующей учётки не перезаписывается.

Супер-админа правит только другой супер-админ и только не себя (`UserService.requireCanManage`): свою учётку через панель не отключить, чужую супер-админскую — можно. Ради этого список и сделан: уходящему сотруднику есть кому закрыть вход. Назначить супер-роль через панель по-прежнему нельзя — источник истины только `ADMIN_EMAIL`.
