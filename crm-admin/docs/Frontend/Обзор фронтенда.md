...
Порядок подключения скриптов в `<head>` (`index.html`, строки 7–17): CDN `Chart.js 4.4.1` и `xlsx 0.18.5` (для «Панели отклонений»), затем `api.js`, `admin-users.js`, `combobox.js`, `wizard.js`, `drawflow.min.js` и `journeys.js` (с `defer`). `api.js` грузится **до** инлайновых скриптов v1 — они рассчитывают, что глобальный объект `window.CRM` уже существует.

## Разделы и навигация

Каждый раздел — `<section class="view" id="...">` в `<main>`; активный получает класс `.active`. Список разделов — двухуровневое дерево `NAV` (`index.html`, ~строка 3395): подробная таблица — в [[Оболочка панели (shell)]]. Кратко:
...
sequenceDiagram
    ...
    A->>S: GET /api/me
    A->>S: GET /api/env
    Note over B: инлайн-скрипт строит сайдбар renderNav()<br/>(пункты с envs скрыты)
...
- данные: встроенный `SEED_RAW` (JSON прямо в `index.html`) либо загрузка/дозагрузка Excel/CSV через SheetJS (`xlsx.full.min.js`);
