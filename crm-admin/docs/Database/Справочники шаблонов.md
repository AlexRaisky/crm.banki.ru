...
Справочник читают два раздела: карточка шаблона (поле «Partner name») и планирование промо (подсказка поля «Партнёр»). Зашитый массив `PROMO_PARTNERS` в `promo.js` остался только фолбэком на случай недоступного справочника и больше не правится.
