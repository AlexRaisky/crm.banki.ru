...
### app.deploy_log — журнал выкаток

`V46__deploy_log.sql`. Кто, когда и до какого коммита катил. Запись появляется в статусе `planned`, когда человек берёт команду, и закрывается в `done`, когда панель видит на целевом контуре запланированную версию: команду выполняют в терминале, и «нажал» ещё не значит «доехало».

### app.settings_snapshot — слепки перед переносом настроек

`V47__settings_pack.sql`. Пакет настроек перезаписывает роли, модель схемы и справочники целиком, и отменить это иначе нельзя. Перед каждым применением сюда кладётся текущее состояние объекта ровно в том виде, в каком его вернул бы экспорт, — значит откат это применение слепка обратно. Таблица append-only: слепков мало, а нужны они как раз через неделю после переноса.
