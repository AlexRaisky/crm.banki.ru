...
| `status` | `varchar(40)` | NOT NULL DEFAULT `'идёт'` | **`V27` (31.07)**, набор расширен в **`V28`**. Проставляется вручную: `запланировано` / `идёт` / `завершён` (по умолчанию `идёт`). Раньше «идёт» выводилось из пустой `date_end`, но статус нужнее отдельно — тест могут запланировать заранее или приостановить |
| `timestamp_cr` | `timestamptz` | NOT NULL DEFAULT now() | Создание |
| `timestamp_upd` | `timestamptz` | NOT NULL DEFAULT now() | Обновление; служит **версией строки** — правка присылает его обратно, при расхождении API отдаёт 409 |
| `created_by` / `updated_by` | `varchar(200)` | — | Авторы |
