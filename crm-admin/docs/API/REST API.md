...
| `POST /api/events/chains` | завести цепочку: строка на шаг в `commapi.events_chain`. Только новые — у события с уже заведённой цепочкой отказ 409 | ADD в `ev-online` |
