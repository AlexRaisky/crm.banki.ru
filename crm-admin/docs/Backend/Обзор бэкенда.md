...
| `AbTestController` | `/api/ab-tests` | общая таблица А/Б тестов — [[АБ тесты]] |
...
Контроллеры тонкие: проверка доступа и делегирование. Транзакции — на сервисном слое (`@Transactional`, читающие с `readOnly = true`). `open-in-view: false`, `ddl-auto: none` — схему создаёт только Flyway (миграции доросли до **V25**), см. [[Схема БД]].
