...
### Три проверки перед сохранением
...
**Дубль** — `requireNotDuplicate`. Проверок две, разной строгости. `letteros_id` (только письма) — точный ключ макета: одно письмо в Letteros живёт в одном шаблоне, совпадение здесь осмысленным не бывает, и обойти его нельзя — `409 «Письмо с letteros_id … уже заведено — шаблон …»`. `source` (колонка `campaign_name`, сравнение без учёта регистра и краевых пробелов) — обычно совпадение означает повторное нажатие «Создать», но у А/Б-пары и у дней цепочки имя кампании общее по замыслу, поэтому отказ снимается параметром `?force=true`: `409 «Шаблон с таким source уже есть — …»`. В `createChain` первый день проверяется полностью, остальные идут с `force` — иначе цепочка упиралась бы в собственный первый шаблон.

Мастер спрашивает `GET /api/templates/{channel}/duplicates` **до** сохранения и показывает вопрос («создать ещё один?») вместо отказа после нажатия. При правке уже открытого шаблона проверка молчит — иначе она спрашивала бы «такой уже есть» про него же самого.
