...
--
--
-- ВНИМАНИЕ про доллар-кавычки и Flyway. Метка $ent$ обязана заканчиваться переводом
-- строки перед JSON: иначе доллар метки и открывающая фигурная скобка встают рядом,
-- Flyway читает это как плейсхолдер, требует значение и падает на разборе файла,
-- до всякой базы. Такую же пару нельзя писать и в комментарии — читатель файла один
-- на весь текст, комментарии он не пропускает.
-- Приложение при этом не поднимается вовсе: миграции идут раньше веб-контекста.
--
