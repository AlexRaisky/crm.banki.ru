 ...
 *
 * Пересечение считается В РАМКАХ КАМПАНИИ: конфликт — когда одна и та же кампания
 * в один день бьёт по одной базе дважды. Разные кампании по общей базе — обычное
 * дело планирования, подсвечивать их нечего. Канал в счёт не идёт: строки одной
 * кампании как раз и различаются каналами.
...
function promoComputeClash(){
  ...
  /* В корзине держим и саму базу, и строки: выковыривать её обратно из составного
     ключа значило бы гадать, где кончается кампания и начинается база. */
  var byDay = {};
  PROMO_ROWS.forEach(function(r, i){
    ...
    bases.forEach(function(b){
      ...
      var k = r.d + '||' + camp + '||' + low;
      var slot = byDay[k] = byDay[k] || { base: low, rows: [] };
      slot.rows.push(i);
    });
  });
  ...
  Object.keys(byDay).forEach(function(k){
    var slot = byDay[k];
    if (slot.rows.length < 2) return;            /* база встречается один раз — не конфликт */
