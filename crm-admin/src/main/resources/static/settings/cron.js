/* ============================================================
   ...
   никогда. Здесь настраивается связь, через которую это будет исправлено.

   Пока экран умеет ровно три вещи: показать настройки, сохранить их и проверить связь.
   Действий над заданиями Quartz нет намеренно — сначала надо убедиться, что панель
   вообще дотягивается до сервиса и чем аутентифицируется. Обратный порядок означал бы
   отлаживать боевой планировщик.

   Токен экран не показывает никогда: сервер отдаёт только признак «задан». Пустое поле
   при сохранении значит «не менять», а не «стереть» — для стирания есть отдельная
   кнопка. Иначе правка адреса молча уносила бы токен, которого человек не видел.
   ============================================================ */
window.Cron = (function () {
  ...
  function render() {
    ...
    host.innerHTML =
      '<div class="cron-card">' +
        ...
        '<label class="cron-chk"><input type="checkbox" id="cron-on"' +
          (data.enabled ? " checked" : "") + "> " + T("Интеграция включена") +
          '<span class="cron-hint">' +
  ...
  return {
    open: function () {
      var host = el("cronHost");
      if (host) host.innerHTML = '<div class="cron-hint">' + T("Загружаю…") + "</div>";
    ...
    /* Проверка ходит наружу и записывает результат — поэтому POST, а не GET: повторять
       её обновлением страницы неправильно. */
    check: function () {
      note(T("Проверяю…"));
      run(api("POST", "../api/cron/check").then(function (res) {
        ...
        return res.ok
          ? T("Связь есть. Ответ: ") + String(res.sample || "").slice(0, 120)
