...
public class ProdDbService {
    ...
    /* Второй спутник sms-шаблона: текст в том виде, в каком его согласовывают сотовые
       ...
       Разложение — три замены, и порядок несущий:

       1) ##любая_переменная## -> %w. В значения переменных не смотрим: по имени
          содержимое не угадать, а %w покрывает и буквы, и цифры, и спецсимволы.
          Имя — что угодно, кроме решётки и управляющих символов: в живых шаблонах
          встречаются кириллица и пробелы внутри имени (##Наименование партнера##).
       2) Ссылка целиком -> %w. Ссылкой считается и https://…, и www…, и голый домен
          с путём вида banki.ru/q/32RhjA8. Хвост пути обязан кончаться буквой, цифрой
          или слешем — иначе точка в конце предложения уезжала бы внутрь переменной.
          Ссылки идут ДО чисел: иначе цифры внутри адреса стали бы %d и ссылка
          перестала бы опознаваться.
       3) Числа, кроме дат -> %d. Дату отсекают ограничители: числу нельзя стоять
          сразу после цифры, точки или дефиса и нельзя иметь такую же группу после
          себя. Поэтому 12.05.2026 и 2026-05-12 остаются, а 1500 становится %d.
          Цена решения: сумма с точкой (1500.50) тоже принята за дату и останется
          текстом; в русских смс разделитель обычно запятая, и 1500,50 разбирается.

       Выпускаем ТОЛЬКО одиночные %w и %d — ни %w+, ни %w{1,n}, ни %d+. Поэтому два
       запрета операторов (на %w+ и на две групповые переменные подряд) нарушить
       нечем, и остаётся один предел: не больше двадцати переменных на шаблон. Тексты
       сверх предела не заполняем совсем — такую строку должен написать человек.

       Второй экземпляр выражения живёт в scripts/sms-approved-backfill.sql (разовая
       доливка и репетиция на копиях). Менять их можно только вместе: разъехавшись,
