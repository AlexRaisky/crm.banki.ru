  ...
  function editField(id, rowEl) {
    ...
    vEl.querySelector("[data-ok]").onclick = function () {
      ...
      evReq("PATCH", "/" + encodeURIComponent(id), { field: field, value: value })
        .then(function (res) {
          res = res || {};
          /* Три разных исхода, и путать их нельзя. Задание планировщика правка обновляет
             сама (PATCH /api/v1/event/{id} его же id) — тогда об этом и говорим. Не
             вышло — говорим, что именно осталось несделанным. Строки в crmdb панель не
             трогает: там своя копия, и довозит её перелив. */
          var msg = res.cronWarning
            ? res.cronWarning
            : (res.cron ? "Сохранено. " + res.cron
                        : (res.exported
                            ? "Сохранено. Событие уже в crmdb — правка туда не уехала:"
                              + " перелейте его заново"
                            : "Сохранено"));
          say("evlMsg", msg, (res.cronWarning || res.exported) ? "warn" : "");
          close();
        })
        .catch(function (e) {
          fail("evlMsg", e);
